# AGENTS.md - AI Agent Development Guide for tap-oracle
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from tap-oracle.

### Key Components

1. **Tap Class** (`tap_oracle/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_oracle/client.py`): Handles API communication and authentication
1. **Streams** (`tap_oracle/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_oracle/streams.py`
...
```python
class MyNewStream(tap-oracleStream):
    name = "my_new_stream"
...
1. **SimpleHeaderPaginator**: For APIs using Link headers (RFC 5988)
   ...
   class MyStream(tap-oracleStream):
...
```bash
tap-oracle --config config.json --discover
...
1. Update `config_jsonschema` in `tap_oracle/tap.py`
...
```bash
# .env.example
TAP_TAP-ORACLE_API_URL=https://api.example.com
TAP_TAP-ORACLE_API_KEY=your_api_key_here
TAP_TAP-ORACLE_BATCH_SIZE=100  # New setting
```
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`uv run tap-oracle --version`), Python version, and a minimal reproduction case when reporting bugs.
